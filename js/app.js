(function () {
  ...
  function dragAllowed() { return !searchActive(); }
  ...
  function initSortable() {
    ...
    sortable = new Sortable($('#card-grid'), {
      ...
      onEnd: () => {
        ...
        if (autoSort) { autoSort = false; localStorage.setItem('cardstack.autosort', 'off'); updateSortUI(); toast('Switched to manual order.'); }
        save();
        maybeAutoBackup();
      },
    });
  ...
  function openScanner() {
    ...
    $('#scanner-msg').textContent = 'Line the barcode up in the frame. Tap to focus · hold ~15 cm away.';
    pushOverlay();
    SCAN.start('reader',
      (text, fmt) => {
        $('#f-number').value = text;
